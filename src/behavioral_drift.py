...
def analyze_feature_drift(
    ...
    for feature in FEATURES:
        ...
        effect = standardized_mean_difference(
            uci_values,
            yoochoose_values
        )
...
def compute_rate_drift(feature_name, p1, n1, p2, n2, dataset1_name="UCI", dataset2_name="YOOCHOOSE"):
    """
    Correct drift test for a RATE/PROPORTION (like purchase_rate) - a
    two-proportion z-test, not a KS-test. A KS-test compares two
    distributions of individual observations; a purchase rate is a single
    summary number per dataset, so there's no distribution to feed it.
    """
    ...
    # Cohen's h - the standard effect size for comparing two proportions
    # (the proportion equivalent of Cohen's d for means)
    ...
    return {
        ...
        "KS_P_Value": round(p_value, 10),
        "Z_Statistic": round(z_statistic, 4),
        "Standardized_Mean_Difference": round(cohens_h, 4),
...
def create_distribution_plot(
    ...
    # Use sampling for the very large YOOCHOOSE dataset
    sample_size = min(
        50000,
        len(yoochoose_df)
    )

    yoochoose_sample = yoochoose_df[
        feature
    ].sample(
        n=sample_size,
        random_state=42
...
def create_purchase_rate_plot(
    ...
    rates = [
        uci_df[TARGET].mean() * 100,
        yoochoose_df[TARGET].mean() * 100
    ]
    ...
    output_path = (
        FIGURES_DIR /
        "behavioral_drift_purchase_rate.png"
    )
...
def main():
    ...
    print(
        f"YOOCHOOSE sessions : {len(yoochoose_df)}"
    )
    ...
    purchase_df = analyze_purchase_rate(
        uci_df,
        yoochoose_df
    )

    print(
        purchase_df.round(6).to_string(
            index=False
        )
    )
    ...
    print(
        f" - {FIGURES_DIR / 'behavioral_drift_clicks.png'}"
    )

    print(
        f" - {FIGURES_DIR / 'behavioral_drift_duration.png'}"
    )

    print(
        f" - {FIGURES_DIR / 'behavioral_drift_unique_items.png'}"
    )

    print(
        f" - {FIGURES_DIR / 'behavioral_drift_purchase_rate.png'}"
    )
